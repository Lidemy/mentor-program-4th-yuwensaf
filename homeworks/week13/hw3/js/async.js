...
// 負責去 api 拿資料（抓取前 5 名的遊戲名稱）：用 async, await
async function getGamesApi() {
  ...
  return response.json(); // 最後回傳的 response.json() 是一個 Promise
}

async function getGamesRun() {
  try {
...
function changeGame(thisGame) {
  ...
  getStreamsApiRun(thisGame);
}

// 負責去 api 拿資料（前 20 個實況）：用 async, await
async function getStreamsApi(thisGame) {
  const response = await fetch(
...
async function getStreamsApiRun(thisGame) {
  try {
    const result = await getStreamsApi(thisGame);
...
// 負責處理畫面（把拿到的資料渲染成畫面）
function appendStreams(streamData) {
