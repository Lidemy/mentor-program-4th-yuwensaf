...
$(document).ready(() => {
  ...
  todos.on('keydown', '.input-edit-todo', e => {
    if (e.key === 'Enter') {
      ...
      if (isDone) { // 如果是「已完成的 todo」，就要多加上 .finished 的 class
  ...
  function restoreTodos(data) {
    ...
    for (const todo of data) {
      ...
      if (todo.isFinished) { // 如果是「已完成的 todo」，就加上 .finished 的 class
        todos.append(todoTemplate
          .replace('">', ' finished">')
          .replace(/{id}/g, id)
          .replace('{todo-content}', escape(content))
        );
      } else { // 如果是「未完成的 todo」，就不用加上 .finished 的 class
        todos.append(todoTemplate
          .replace(/{id}/g, id)
          .replace('{todo-content}', escape(content))
        );
